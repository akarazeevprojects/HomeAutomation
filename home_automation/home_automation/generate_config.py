...
def build_parser():
    ...
    parser.add_argument(
        "-r",
        "--replace",
        help="replace ~/.homebridge/config.json with generated newconfig.json",
        action="store_true",
    )
    return parser
...
def generate_newconfig():
    ...
    for fullname, info in system.devices.items():
        ...
        struct = {
            "name": fullname,
            "on_cmd": "python3 {} -d {} -on".format(system.ADAPTER_PATH, name),
            "off_cmd": "python3 {} -d {} -off".format(system.ADAPTER_PATH, name),
            "state_cmd": "python3 {} -d {} -s".format(system.ADAPTER_PATH, name),
        }
